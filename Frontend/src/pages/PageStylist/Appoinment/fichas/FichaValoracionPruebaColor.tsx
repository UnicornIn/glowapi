...
export function FichaValoracionPruebaColor({ cita, datosIniciales, onGuardar, onSubmit, onCancelar, fichaId, modoEdicion }: FichaValoracionPruebaColorProps) {
  ...
  const handleFileSelect = (tipo: 'estado_actual' | 'expectativa', files: FileList | null) => {
    ...
    if (remainingSlots <= 0) {
      toast.warning(`Máximo 5 imágenes permitidas para ${tipo === 'estado_actual' ? 'estado actual' : 'expectativa'}`);
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.servicio_valorado.trim()) {
      toast.warning('Debe especificar el servicio valorado');
      return;
    }

    if (!formData.acuerdos.trim()) {
      toast.warning('Debe describir los acuerdos con el cliente');
      return;
    }

    if (!formData.recomendaciones.trim()) {
      toast.warning('Debe incluir recomendaciones basadas en la valoración');
      return;
    }
