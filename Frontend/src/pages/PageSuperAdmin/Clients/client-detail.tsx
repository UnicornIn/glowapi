...
function HistorialTab({ client, onFichaChanged }: { client: Cliente; onFichaChanged?: () => void }) {
  ...
  if (historial.length === 0 && fichas.length === 0 && paquetes.length === 0) return <div className="glw-empty-state">Sin historial de servicios</div>
    ...
    <>
      {paquetes.length > 0 && (
        <>
          ...
          <table className="glw-hist-table">
            <thead><tr><th>Paquete</th><th>Sesiones restantes</th><th>Comprado</th></tr></thead>
            <tbody>
              {paquetes.map((p) => (
                <tr key={p.paquete_id}>
      ...
      {fichas.length > 0 && (
        <>
                  ...
                  <tr key={fichaKey}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      <div style={{ display: 'inline-flex', gap: 6 }}>
                        ...
                        <button onClick={() => setFichaEditando(ficha)} className="glw-btn glw-btn-sm" style={{ fontSize: 11 }}>
                          Editar
                        </button>
                        <button onClick={() => handleEliminarFicha(ficha)} disabled={eliminandoId === fichaKey} className="glw-btn glw-btn-sm" style={{ fontSize: 11, color: '#b83030', borderColor: '#f0d0d0' }}>
          ...
          {datosUltimaFicha?.recomendaciones_personalizadas && (
            <><div className="glw-s-title">Recomendaciones del especialista</div><div className="glw-nota"><div className="glw-nota-text">{datosUltimaFicha.recomendaciones_personalizadas}</div></div></>
          )}
        </>
      )}
      ...
      {historial.length > 0 && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, marginTop: fichas.length > 0 ? 24 : 0 }}>
